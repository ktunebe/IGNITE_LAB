...
# Private subnets (one per AZ) — ECS tasks live here
...
# Elastic IP + NAT gateway — gives private subnets outbound internet access
...
# Security group: ALB — accepts HTTP from anywhere
...
# Security group: ECS tasks — accepts traffic only from the ALB
